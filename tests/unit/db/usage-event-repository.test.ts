...
describe("UsageEventRepository (P1-13)", () => {
  ...
  describe("createMany", () => {
    ...
    it("bulk inserts with per-row hour buckets", () => {
      const n = ctx.repo.createMany([
        { eventType: "api.call", createdAt: Date.UTC(2026, 4, 28, 10) },
        { eventType: "api.call", createdAt: Date.UTC(2026, 4, 28, 11) },
        { eventType: "api.call", createdAt: Date.UTC(2026, 4, 28, 11) },
      ]);
      expect(n).toBe(3);
      expect(ctx.repo.list({})).toHaveLength(3);
    });
  });
  ...
  describe("aggregate", () => {
    beforeEach(() => {
      ctx.repo.createMany([
        { eventType: "skill.view", quantity: 1, createdAt: Date.UTC(2026, 4, 28, 10) },
        { eventType: "skill.view", quantity: 1, createdAt: Date.UTC(2026, 4, 28, 10) },
        { eventType: "pipeline.run", quantity: 4, createdAt: Date.UTC(2026, 4, 28, 10) },
        { eventType: "skill.view", quantity: 1, createdAt: Date.UTC(2026, 4, 28, 11) },
      ]);
    });
    ...
    it("filters by event type", () => {
      const rows = ctx.repo.aggregate({ eventType: "pipeline.run" });
  ...
  describe("sumQuantity", () => {
    it("returns 0 when no rows match", () => {
      expect(ctx.repo.sumQuantity({})).toBe(0);
    });

    it("sums across all matching rows", () => {
      ctx.repo.createMany([
        { eventType: "storage.write", quantity: 100, createdAt: Date.UTC(2026, 4, 28, 10) },
        { eventType: "storage.write", quantity: 250, createdAt: Date.UTC(2026, 4, 28, 11) },
      ]);
      expect(ctx.repo.sumQuantity({ eventType: "storage.write" })).toBe(350);
    });
  });

  describe("list", () => {
    it("orders by created_at desc and respects limit", async () => {
      await ctx.repo.create({ eventType: "api.call", createdAt: 1000, resourceId: "old" });
      await ctx.repo.create({ eventType: "api.call", createdAt: 2000, resourceId: "mid" });
      await ctx.repo.create({ eventType: "api.call", createdAt: 3000, resourceId: "new" });
      const events = ctx.repo.list({ limit: 2 });
    ...
    it("clamps limit to [1, 10000]", async () => {
      await ctx.repo.create({ eventType: "api.call", createdAt: 1 });
      expect(ctx.repo.list({ limit: 0 })).toHaveLength(1);
      expect(ctx.repo.list({ limit: -5 })).toHaveLength(1);
    });
    ...
    it("returns null metadata when JSON is corrupt rather than throwing", () => {
      ...
      ctx.db.run(`INSERT INTO usage_events
        (id, event_type, quantity, metadata, hour_bucket, created_at)
        VALUES ('id1', 'api.call', 1, '{not-json', '2026-05-28T13', 100)`);
      const events = ctx.repo.list({});
      expect(events).toHaveLength(1);
    ...
    it("returns null metadata when JSON parses to a non-object", () => {
      ctx.db.run(`INSERT INTO usage_events
        (id, event_type, quantity, metadata, hour_bucket, created_at)
        VALUES ('id1', 'api.call', 1, '"a string"', '2026-05-28T13', 100)`);
      const events = ctx.repo.list({});
      expect(events[0].metadata).toBeNull();
    });
  });
  ...
  describe("deleteOlderThan", () => {
    it("deletes rows with created_at < cutoff", async () => {
      await ctx.repo.create({ eventType: "api.call", createdAt: 100 });
      await ctx.repo.create({ eventType: "api.call", createdAt: 500 });
      await ctx.repo.create({ eventType: "api.call", createdAt: 1000 });
