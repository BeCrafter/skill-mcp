...
describe("WebhookDispatcher (P1-16)", () => {
  ...
  it("dispatch success: 2xx → status=success, response captured", async () => {
    ...
    const d = s.deliveryRepo.enqueue({ webhookId: w.id, tenantId: "default", eventType: "skill.published", payload: '{"x":1}' });
  ...
  it("dispatch sends signature + delivery-id headers", async () => {
    const w = s.service.create({ tenantId: "default", url: "https://e.x/h", eventTypes: ["skill.published"] });
    const d = s.deliveryRepo.enqueue({
  ...
  it("4xx (non-408/429) → dead_letter, no retry", async () => {
    const w = s.service.create({ tenantId: "default", url: "https://e.x/h", eventTypes: ["skill.published"] });
    const d = s.deliveryRepo.enqueue({ webhookId: w.id, tenantId: "default", eventType: "skill.published", payload: "{}" });
  ...
  it("dispatchDue iterates over all due rows", async () => {
    const w = s.service.create({ tenantId: "default", url: "https://e.x/h", eventTypes: ["skill.published"] });
    s.deliveryRepo.enqueue({ webhookId: w.id, tenantId: "default", eventType: "skill.published", payload: "{}", now: 100 });
    s.deliveryRepo.enqueue({ webhookId: w.id, tenantId: "default", eventType: "skill.published", payload: "{}", now: 200 });
    const fetchImpl = makeFetchOk();
    const dispatcher = new WebhookDispatcher(s.webhookRepo, s.deliveryRepo, s.service, s.logger, {
      fetchImpl, now: () => 500,
    });
