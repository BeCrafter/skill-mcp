...
| 修改普通用户 | ✅ | ✅ | |
| 修改管理员用户 | ✅ | ❌ | 必须 superadmin（admin 不可改任何 admin，含自己） |
| 修改超级管理员 | 仅自己 | ❌ | 超管互保；仅可自改 |
| 删除普通用户 | ✅ | ✅ | |
| 删除管理员 | ✅ | ❌ | 必须 superadmin |
| 删除超级管理员 | ❌ | ❌ | **任何人都不行，含自删** |
| 自删 | ❌ | ❌ | **任何人都不行**（HTTP 与 CLI 均拦截） |
| 分配 superadmin/admin 角色 | ✅ | ❌ | 必须 superadmin |
| 分配 user/其他自定义角色 | ✅ | ✅ | |
| 轮换自己的 token | ✅ | ✅ | |
| 轮换普通用户 token | ✅ | ✅ | |
| 轮换管理员 token | ✅ | ❌ | 必须 superadmin |
| 轮换超级管理员 token | 仅自己 | ❌ | 超管互保；仅可自轮换 |
| 重置自己的密码 | ✅ | ✅ | 仅 HTTP `/api/auth/change-password`（old+new）；CLI 无 change-password，`auth reset-password` 不可自重置 |
| 重置管理员密码 | ✅ | ❌ | 必须 superadmin（CLI `auth reset-password`） |
| 重置超级管理员密码 | 仅自己 | ❌ | 超管互保；仅可自重置 |
...
| 守卫函数 | 位置 | 作用 |
|----------|------|------|
| `enforceAdminAuth` | route-group 级（`/api/admin/*` + `/metrics`） | 验证 token，限制 `userType` 为 admin 或 superadmin |
| `enforceGatewayAuth` | route-group 级（`/api/gateway/*`） | 验证 bearer token，解析请求上下文 |
| `requireSuperadmin(rc)` | 端点级 | 拒绝非 superadmin 调用 |
| `assertCanOperateOn(target, operatorId, operatorType, mutate)` | HTTP 端点级（`src/http/middleware/admin-auth.ts` 调用方 `users.handler.ts`） | 用户变更目标校验：超管互保（仅可操作自身）、admin 不可操作任何 admin（含自己，需 superadmin）。`mutate=false`（读取）时 admin 可查看自己；`mutate=true`（修改/轮换）时 admin 自我也拒绝。**自删由 DELETE handler 单独一律拒绝** |
| `assertCanOperateOnCli(..., mutate)` | CLI 层（`src/cli/commands/user-cmd.ts`） | 同上的 CLI 对应实现；`userDeleteAction` 额外拦截自删（HTTP 与 CLI 一致） |
...
### TC-13: 管理员不能轮换任何管理员 token（含自己，需 superadmin）

```bash
skillmcp user rotate-token ${admin_id}  # → 应拒绝（自轮换亦禁止）
```
...
### TC-19: 管理员不能修改任何管理员用户（含自己，需 superadmin）
...
### TC-21: 管理员不能删除管理员（含自删；HTTP 与 CLI 均拦截）

```bash
skillmcp user delete ${admin_id}  # → 应拒绝（CLI 拦截自删）
curl -X DELETE /api/admin/users/${admin_id} -H "Authorization: Bearer ${admin_token}"  # → 应返回 403（HTTP 拦截自删）
```
...
### TC-25: 重置自己密码（HTTP change-password；CLI auth reset-password 仅超管可自重置）

```bash
# 超管重置自己（CLI 允许）
skillmcp auth reset-password --username admin --password newpass123  # → 应成功
# 管理员重置自己（CLI 拒绝：仅 superadmin 可 reset admin 密码）
skillmcp auth reset-password --username admin01 --password newpass123  # → 应拒绝
# 管理员自改密钥走 HTTP（需 old_password）
curl -X POST /api/auth/change-password -H "Authorization: Bearer ${admin_token}" -d '{"old_password":"admin888","new_password":"newpass123"}'  # → 应成功
```
